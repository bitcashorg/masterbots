...
COMMENT ON TABLE public.thread IS 'This is a table of conversations between user and chatbot.  A thread is created every time user clicks New chat.';
...
ALTER TABLE ONLY public.message
    ADD CONSTRAINT message_thread_id_key UNIQUE (thread_id);
...
ALTER TABLE ONLY public.thread
    ADD CONSTRAINT thread_chatbot_id_key UNIQUE (chatbot_id);
ALTER TABLE ONLY public.thread
    ADD CONSTRAINT thread_pkey PRIMARY KEY (thread_id);
ALTER TABLE ONLY public.thread
    ADD CONSTRAINT thread_user_id_key UNIQUE (user_id);
...
ALTER TABLE ONLY public.thread
    ADD CONSTRAINT thread_user_id_fkey FOREIGN KEY (user_id) REFERENCES public."user"(user_id) ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE ONLY public.user_chatbot_preference
