	...
	chatbotMetadataExamples: {
		...
		parameters: z.object({
			chatbot: z.object({
				...
				domainId: z
					.number()
					.describe('The domain ID of the chatbot, ranging from 1 to 3.'),
			}),
			userContent: z
				.string()
				.describe(
...
export const metadataSchema = z.object({
	categories: z
		.array(z.string())
		.describe('Exact category name from provided options.'),
	subCategories: z
		.array(z.string())
		.describe('Exact subcategory name from chosen category.'),
	tags: z
		.array(z.string())
		.describe('Array of exact tag names from provided options.'),
})
...
export const languageGammarSchema = z.object({
	...
	improved: z
		.boolean()
		.describe('Indicates if the text was improved or viewed.'), // false
})
