...
export function createImprovementPrompt(content: string): string {
    ...
    Follow these steps:
    1. Identify the original language of the provided text. 2. Correct clear typos in common words based on the intended meaning. If the input is ambiguous or appears to be intentionally unconventional, preserve it as is. 3. Correct spelling errors and fix obvious grammar issues while keeping the original tone and meaning. 4. Adjust punctuation where needed, but only when it's clearly incorrect or missing. 5. Provide the final corrected text in the original language, ensuring it retains the intended meaning and structure.
    **Important Guidelines:**
    - For very short inputs or single words, avoid making changes unless the correction is absolutely certain. - Maintain the original structure and formatting of the input as much as possible. - Output only the corrected and improved text, without any additional explanations. - Include the flag whether the text was improved or not. - Provide both the original and translated question only if original is different from 'en' (English).
    ## Example: ##
...
export function createChatbotMetadataPrompt(
	...
	return `You are a top software development expert with extensive knowledge in the field of ${metadataHeaders.domain}. Your sole purpose is to label the following question "${userPrompt}" with the appropriate categories, sub - categories and tags as an array of strings. These are the available categories, sub-categories and tags:${chatbotMetadata.questions}${chatbotMetadata.categories}${chatbotMetadata.subCategories}${chatbotMetadata.tags}**Important Guidelines:**
    - Output only the requested fields without any additional explanation. - Provide the labels in the exact format as requested.
    ## Example: ##
...
export function createBotConfigurationPrompt(chatbot: Chatbot) {
	return `Your response tone will be ${chatbot.defaultTone}. Your response length will be ${chatbot.defaultLength}. Your response format will be ${chatbot.defaultType}. Your response complexity level will be ${chatbot.defaultComplexity}. Your response will be generated in the same language as user input.
    **Important Guidelines:**
    - Do not change the response tone, length or complexity level, only format whenever requested as additional instructions and/or examples. - You may be capable of performing Web Search. When available, use it to verify information before making assumptions. `
