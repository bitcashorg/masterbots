  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 right-0 flex justify-between items-center w-full h-full px-[4px]">
        <Eye className={`w-[16px] h-[16px] transition-opacity duration-300 ${isChecked ? 'text-gray-400' : 'text-gray-200'}`} />
        <EyeOff className={`w-[16px] h-[16px]  transition-opacity duration-300 ${isChecked ? 'text-gray-200' : 'text-gray-400'}`} />
      </div>
    </Switch.Root>
  );
}
