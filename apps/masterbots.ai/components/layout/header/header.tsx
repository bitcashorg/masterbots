...
function HeaderLink({
  ...
  const isActive =
    ...
    (href === '/' && routeType === 'public' && pathname.length > 1 && !pathname.startsWith('/c')) ||
    // For personal/chat route: href is "/c" and pathname starts with "/c/"
    (href === '/c' && routeType === 'chat' && (pathname === '/c' || pathname.startsWith('/c/')))
  const routeColour = getRouteColor(isActive, pathname)

  return (
    <Button
      className={cn('-ml-1 transition-all', { [`${routeColour}`]: isActive && !noActiveColor })}
      onClick={onClick}
      variant="link"
      asChild
    >
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between w-full h-16 px-4 border-b shrink-0 bg-gradient-to-b from-background/10 via-background/50 to-background/80 backdrop-blur-xl">
      <div className="flex items-center">
        ...
        <HeaderLink href="/" noActiveColor onClick={resetNavigation} text="MB" />

        {/* Navigation links - Hidden on mobile */}
        <div className="hidden md:flex lg:items-center">
          <IconSeparator className="size-6 text-muted-foreground/50" />
          <HeaderLink href={personalUrl} onClick={resetNavigation} text="Chat" />
          <HeaderLink href={publicUrl} onClick={resetNavigation} text="Public" />
