...
import { PreferenceSectionProps } from '@/types/types';

export  function  PreferenceSection({title, items, variant}: PreferenceSectionProps){
    return (
                  ...
                  <Card className="bg-transparent border-mirage">
                    <CardContent className="px-4 py-8 flex flex-col justify-center items-center gap-y-4 w-full">
                      {items.map((item, idx) => (
                        <div
                          key={idx}
                          className={cn("flex justify-between items-center gap-x-5 px-4 border-b border-mirage pb-5 w-full",
                            idx === items.length - 1 ? "border-none" : ""
                          )}
    
                        >
                         
                           <PreferenceItemTitle title={item.title} description={item.description}  />
                          {item.type === 'switch' && (
                            <Switch
                              defaultChecked={
                                'defaultChecked' in item
                                  ? item.defaultChecked
                                  : false
                              }
