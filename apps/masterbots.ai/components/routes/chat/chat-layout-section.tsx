...
export function ChatLayoutSection({ children }: { children: React.ReactNode }) {
  ...
  const chatClassNames = activeChatbot ? 'max-h-[calc(97vh-210px)] md:max-h-[calc(97vh-182px)]' : 'max-h-[calc(97vh-26px)]'

  return (
    <section
      ref={sectionRef as React.Ref<HTMLDivElement>}
      className={cn(
        isPublic ? 'max-h-[calc(97vh-156px)] md:max-h-[calc(97vh-100px)]' : chatClassNames,
    ...
    >
      <div className="flex flex-col w-full max-w-screen-lg gap-10 px-4 pt-5 mx-auto h-full md:px-10">
        {children}
      </div>
