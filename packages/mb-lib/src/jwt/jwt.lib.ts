...
import type { JwtData } from 'mb-types'
import { getErrorMessage } from '../error'
import { generateHasuraClaims } from '../hasura'
import type { JwtSecret, TokenLibGetTokenParams } from './jwt.type'
...
export function validateJwtSecret(envVariable: string | undefined): JwtSecret {
  ...
  if (
    ...
    !['HS256', 'HS238', 'HS512', 'RS256', 'RS384', 'RS512', 'Ed25519'].includes(secret.type)
  ) {
  ...
  if (
    ...
    (typeof secret.claims_namespace !== 'string' || secret.claims_namespace.length === 0)
  ) {
