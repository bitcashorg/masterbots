...
export const preference = pgTable(
	'preference',
	{
		...
		chatbotId: integer('chatbot_id').default(1),
...
export const user = pgTable(
	'user',
	{
		...
		proUserSubscriptionId: text('pro_user_subscription_id')
			.default('')
			.notNull(),
...
export const example = pgTable(
	...
	(table) => [
		foreignKey({
			...
			foreignColumns: [
				subcategoryEnum.name,
				subcategoryEnum.category,
				subcategoryEnum.domain,
			],
...
export const socialFollowing = pgTable(
	...
	(table) => [
		index('idx_social_following_followee').using(
			'btree',
			table.followeeId.asc().nullsLast().op('uuid_ops'),
		),
		index('idx_social_following_follower').using(
			'btree',
...
export const chatbotCategory = pgTable(
	...
	(table) => [
		...
		foreignKey({
			...
			.onDelete('restrict'),
		primaryKey({
			columns: [table.chatbotId, table.categoryId],
			name: 'chatbot_category_pkey',
		}),
	],
)

export const promptChatbot = pgTable(
	'prompt_chatbot',
	{
	...
	(table) => [
		...
		foreignKey({
			...
			.onDelete('restrict'),
		primaryKey({
			columns: [table.promptId, table.chabotId],
			name: 'prompt_chatbot_pkey',
		}),
	],
)

export const promptUser = pgTable(
	'prompt_user',
	{
		userId: uuid('user_id').notNull(),
		promptId: integer('prompt_id').notNull(),
...
export const chatbotDomain = pgTable(
	'chatbot_domain',
	{
		chatbotId: integer('chatbot_id').notNull(),
	...
	(table) => [
		...
		foreignKey({
			...
			.onDelete('restrict'),
		primaryKey({
			columns: [table.chatbotId, table.domainName],
			name: 'chatbot_domain_pkey',
		}),
	],
)

export const categoryEnum = pgTable(
	'category_enum',
	{
		name: varchar().notNull(),
		domain: varchar().notNull(),
	...
	(table) => [
		foreignKey({
			...
			.onDelete('restrict'),
		primaryKey({
			columns: [table.name, table.domain],
			name: 'category_enum_pkey',
		}),
	],
)

export const subcategoryEnum = pgTable(
	'subcategory_enum',
	{
		name: varchar().notNull(),
		category: varchar().notNull(),
	...
	(table) => [
		foreignKey({
			...
			.onDelete('restrict'),
		primaryKey({
			columns: [table.name, table.category, table.domain],
			name: 'subcategory_enum_pkey',
		}),
	],
)
...
export const message = pgTable(
	...
	(table) => [
		...
		foreignKey({
			...
			.onDelete('cascade'),
		primaryKey({
			columns: [table.messageId, table.slug],
			name: 'message_pkey',
		}),
